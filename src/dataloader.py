...
class VisionDataset(object):
	...
	def get_loader(self, indices, transforms, train, shuffle=True, target_transforms=None):
		...
		train_ds = torchvision.datasets.CIFAR100(root=self.opt.data_dir, train=True, download=True, transform=transforms, target_transform=target_transforms)
		valid_ds = torchvision.datasets.CIFAR100(root=self.opt.data_dir, train=False, download=True, transform=transforms, target_transform=target_transforms)
		...
		self.forget_valid_dl = DataLoader(forget_valid, 8, num_workers=0, pin_memory=True)

		self.retain_valid_dl = DataLoader(retain_valid, 8, num_workers=0, pin_memory=True)
		...
		if self.opt.dataset in ['CIFAR10', 'CIFAR100', 'MNIST', 'KMNIST', 'FashionMNIST']:
			if train:
				return DataLoader(retain_train, sampler=sampler, **self.kwargs)
			else:
				return DataLoader(retain_train, sampler=sampler, **self.kwargs)
	...
	def gen_cl_mapping(self):
		...
		self.forget_valid_dl = self.forget_valid_dl
		self.retain_valid_dl = self.retain_valid_dl
